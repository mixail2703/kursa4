...
struct record//структура элемент массива структру или элемент очереди или элемент дерева 
{
	char FIO[30];
...
struct vertex// структура дерева в каждом элементе которого есть указатели и структура типа  record 
{
	record *data;
	vertex *next;
	int bal;
...
int main()
{
	...
	f=fopen("C:/Users/Kiddacloud/Desktop/kursa4/testBase2.dat", "rb");
	list *base = new list;
...
void CodeBase() {
	FILE *f, *fcoded;
    f = fopen("C:/Users/Kiddacloud/Desktop/kursa4/testBase2.dat", "rb");
    fcoded = fopen("C:/Users/Kiddacloud/Desktop/kursa4/BaseCoded.dat", "wb");
	char buffer;
	while (!feof(f)) {
		fscanf(f, "%c", &buffer);
		fcompression++;
